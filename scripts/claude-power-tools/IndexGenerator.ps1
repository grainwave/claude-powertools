...
# ============================================
# Generate CLAUDE.md in docs folder
# ============================================
Write-Host "Generating CLAUDE.md for architecture context..." -ForegroundColor Cyan
Write-Log "Generating CLAUDE.md in docs folder"

$claudeMdPath = Join-Path $DocsPath "CLAUDE.md"
$claudeMdContent = @"
# Architecture Context for $RootFolderName

This file provides guidance to Claude Code when working with projects in this repository.

## Important: Repository Architecture

**ALWAYS review the index.md file in this folder before responding to prompts.**

The index.md file contains:
- Comprehensive overview of all projects in this repository
- Architecture patterns and design approaches
- Dependency relationships between projects
- Integration points and how projects relate to each other
...
1. **Consider the Overall Architecture**: Review index.md to understand how the current project fits into the broader repository architecture

2. **Check Internal Dependencies**: Before making changes, verify if the project has internal dependencies on other projects listed in index.md

3. **Maintain Consistency**: Follow the architectural patterns and design approaches documented in index.md

4. **Cross-Project Impact**: If a change might affect other projects (e.g., NuGet package updates, API changes), reference the dependency information in index.md

5. **Integration Awareness**: Be aware of how projects integrate with each other as documented in index.md

## Context

This repository contains multiple related projects that work together. Understanding the relationships between projects (documented in index.md) is crucial for making informed decisions about code changes, refactoring, and new features.

---

*This file was automatically generated by Claude PowerTools. To update, regenerate the index by deleting index.md and running the Claude Session Manager again.*
"@
...
try {
    ...
    Write-Host ""
    Write-Host "Architecture context will now be included in all Claude sessions!" -ForegroundColor Green
    Write-Log "Successfully generated CLAUDE.md at: $claudeMdPath"
}
catch {
    Write-Host "  ✗ Error writing CLAUDE.md: $($_.Exception.Message)" -ForegroundColor Yellow
    Write-Log "Error writing CLAUDE.md: $($_.Exception.Message)" "WARN"
